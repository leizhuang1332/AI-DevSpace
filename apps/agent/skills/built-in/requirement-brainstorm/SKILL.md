...
你的输出会被 `analysis-chunk-parser` **逐行**解析。解析器识别的合法标记:
`[SUBPROBLEM]` / `[RISK]` / `[OPTION]` / `[SUBPROBLEM_EMPTY]` 等。

- ✅ 标记**行首出现即可** —— 推荐两种等价写法任选其一:
  - **内联**(更紧凑):`[SUBPROBLEM] 尾差落点未指定节点。` (同一条产物一行写完)
  - **多行**(示例风格):标记独占一行,下一行 `text: <正文>`,再下 `source_refs:`
- ✅ 标记必须**顶格**;前面允许 `**` / `__` 加粗包裹(`**[RISK]**` 也认)
...
**本轮唯一合法的输出形态**:若干个「以标记开头的段落」依次排列。除此之外不要输出任何内容。
...
```
[SUBPROBLEM] 单笔退款金额上限是否随用户等级差异化?
source_refs:
  - prd:8-12 "退款单笔金额上限 ≤ 1000 元"

[RISK] 现有退款审核流依赖财务人工,新增 1000 元以下免审路径可能导致风控盲区。
source_refs:
  - prd:14-18 "退款审核流由财务人工审核"

[OPTION] 异步多阶段事件驱动 + 幂等网关,可将退款入口 RT 控制在 80ms 内。
source_refs:
  - prd:22-26 "退款失败时回滚优惠券 / 库存"
```

**写法 B · 标记独占行 + 显式 `text:` 字段(也支持)**:
