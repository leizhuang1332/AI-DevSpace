...
- ❌ 任何**没有标记**的行一律被降级为 `kind: 'narration'`,**不会**点亮 AdmissionDashboard 五维卡
- ❌ 严禁输出 markdown 标题(`#` / `##`)、表格(`|`)、有序/无序列表(`1.` / `-`)、
  分隔线(`---`)—— 这些都不是标记,会导致整轮准入结果为空
- ❌ 不要写导语("以下是准入校验结果")、结尾总结、"综合判定"等包裹性文字

**本轮唯一合法的输出形态**:5 个 `[DIM ...]` 段 + 1 个 `[VERDICT]` 段,段间空行分隔。
除此之外不要输出任何内容。
