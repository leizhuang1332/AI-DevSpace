...
describe('checkStatusConstraint — implementing rule', () => {
  ...
  it('assumes the caller has already filtered archived (per ADR-0025 D6)', () => {
    // ADR-0025 D6 + issue 03 ticket 5:"约束校验用 TaskCardStore.list
    // (过滤非 archived)实现,不直读文件系统"——Guard 假设 cards 已经是
    // 非 archived 子集;若调用方传入 archived 卡则视为活跃卡参与校验。
    ...
    // Guard 不自行过滤 archived —— 委托给 store.list 过滤
    expect(result.ok).toBe(false)
  })
})
...
describe('checkStatusConstraint — submitting rule', () => {
  ...
  it('Guard does not filter archived — relies on caller (caller uses store.list default filter)', () => {
...
describe('checkStatusConstraint — done rule', () => {
  ...
  it('Guard does not filter archived on done rule — relies on caller filter', () => {
