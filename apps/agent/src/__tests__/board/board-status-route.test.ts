...
beforeEach(async () => {
  ...
  // 也显式提供 setter 桩,让反向不约束测试可以断言"route 没有调 parent writer"
  setStatusSpy = vi.fn()
  updateSpy = vi.fn()
  requirementService = {
    get: vi.fn(() => ({ id: REQ_ID })),
    ...
    // 反向不约束测试用:实际断言里 spy 不被调
    setStatus: setStatusSpy,
    updateRequirement: updateSpy,
...
describe('PATCH /board/cards/:cardId/status — reverse direction does not mutate parent', () => {
  it('does NOT mutate the parent status (no setter on RequirementService is invoked)', async () => {
    ...
    // ADR-0025 D3:route 只读 parent status(派生),不应有 setter 调用。
    // 用显式 spy 验证(fake 上挂 setStatus / updateRequirement,断言 spy 不被调)。
    // 之前用 Object.keys 启发式只能证明 fake 碰巧没 setter —— 不验证 route 代码。
