...
function writeAnalysisBundle(id: string, opts: { skipIndex?: boolean; skipChunks?: boolean } = {}): void {
  const sessionsDir = join(tmpRoot, id, 'analysis', 'sessions')
...
function writeDesignDir(id: string): string {
  const dir = join(tmpRoot, id, 'design')
...
describe('ANALYZING · getAnalyzingData · 显式 options 覆盖', () => {
  ...
  it('显式 lastSessionId 命中 sessions 列表 → activeSessionId 取显式值', async () => {
    mkdirSync(join(tmpRoot, 'req-multi', 'analysis', 'sessions'), { recursive: true })
    writeFileSync(
      join(tmpRoot, 'req-multi', 'analysis', 'sessions', '_index.yaml'),
      [
        'sessions:',
        '  - id: sess-a',
        '    label: A',
