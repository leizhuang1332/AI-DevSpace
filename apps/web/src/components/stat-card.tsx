...
export function StatCard({ label, value, delta, deltaTone = 'neutral', valueClassName }: Props) {
  return (
    <div className="bg-bg-elevated border border-border rounded-lg p-5">
      <div className="text-text-3 text-sm mb-2">{label}</div>
      <div className={`text-[32px] font-semibold tracking-tight leading-none ${valueClassName ?? ''}`}>{value}</div>
