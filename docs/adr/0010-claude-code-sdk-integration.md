...
**采用 官方 TypeScript SDK `@anthropic-ai/claude-agent-sdk`（[npm](https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk)）**

- Agent 端 `import { query } from '@anthropic-ai/claude-agent-sdk'`
...
├── providers/
│   ├── AIProvider.ts            # 抽象接口
│   ├── ClaudeCodeProvider.ts    # @anthropic-ai/claude-agent-sdk SDK 实现
