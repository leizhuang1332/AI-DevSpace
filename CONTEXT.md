...
| 124 | **拖拽触发器 = 左侧 hover 才显手柄**：`⋮⋮` 6 点 sprite；默认 `opacity: 0`；`group-hover:opacity-100`；激活区域 = 5px 移动阈值（@dnd-kit `PointerSensor` 默认 `activationConstraint.distance`）；移动 < 5px = 当点击进详情 | [ADR-0035](docs/adr/0035-board-drag-sort.md) D4(v1) |
| 128 | **拖拽触发器 v2 = card-top 整行**(D4 v2 替换 D4 v1)：触发器 = `card-top` 整行(id 短哈希 + 菜单 `⋯`),`useSortable` listeners 绑到此 div;hover 反馈 = `cursor:grab` + `background: brand-50` + `box-shadow: inset 0 2px 0 brand`(Linear 风格 2px 顶线);focus 反馈 = `.card:focus-visible` 显 `outline:2px brand` + `outline-offset:2px` + `box-shadow:0 0 0 4px brand-50`;不可拖区 = title 主体 + meta 行(click 进详情);拖拽中 ghost = `scale(1.04) rotate(2deg)` + brand-700 阴影;目标列空列 + 跨列拖 → 显示 120px placeholder「释放此处 →」;非空列 + 跨列拖 → 现有卡片 `translateY(8px) + opacity:0.7`,200ms ease-out 让位 | [ADR-0035](docs/adr/0035-board-drag-sort.md) D4(v2) |
