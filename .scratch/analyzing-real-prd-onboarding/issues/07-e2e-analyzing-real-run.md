...
- [x] 新增 `apps/web/e2e/analyzing-real-run.spec.ts`
- [x] spec 步骤:
...
- [x] spec 启动时检查 `process.env.ANTHROPIC_API_KEY`;缺时 `test.skip()` 并打印 SKIPPED 日志行(明确告知原因)
- [x] CI 默认跑(若 secret 设置);开发机可直接 `pnpm e2e` 跑
- [x] 提供 reviewer 手工验收脚本(`scripts/verify-analyzing-real-run.sh`):跑一次真 SDK 并把 SSE / chunks.jsonl 头几行归档到 `~/.aidevspace/verification/`
- [x] `pnpm typecheck` 与 `pnpm test` 通过;e2e 在 CI 默认 SKIP 时不 fail
