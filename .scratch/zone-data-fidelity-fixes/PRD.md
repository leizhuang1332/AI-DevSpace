...
# 工位数据保真度修复 — DRAFTING/ANALYZING/DESIGNING 真实数据驱动 + 标题字段只读化

**What to build:** 让 6 个工位的 mock 数据层对齐真实文件系统产物;同时把 DRAFTING 工作台里冗余的"标题"输入框改为只读展示,并把启动校验从"标题+PRD"简化为"只校验 PRD"。四件事一起解决,因为它们共享同一个根因(mock loader 把真实需求当作空需求)。
...
- **D-1.1** 新建 `apps/web/src/lib/drafting.server.ts`,导出 `getDraftingDataFromFs(reqId)`:
  - 路径解析:仓库根 = `process.cwd()` 在 dev 时是 `apps/web/`,拼 `path.resolve(process.cwd(), '../../requirements/{reqId}/requirement.md')`
...
- **D-1.2** 新建 `apps/web/src/lib/designing.server.ts`,导出 `getDesigningDataFromFs(reqId)`:
  - 路径:`requirements/{reqId}/design/` 目录存在 + 内部至少一个候选方案 yaml 存在 → 非空;否则 `emptyDesigning(reqId)`
...
- **O-8**:production 部署的 `process.cwd()` 路径处理(dev 时 cwd = `apps/web/`;production 部署路径留 TODO 注释,本期不解决)
...
### N-2:dev / production 路径处理

dev 时 `process.cwd() === <repo-root>/apps/web`,路径拼 `'../../requirements/{id}/...'` 正确;production 部署时 cwd 可能是仓库根或子目录,**留 TODO 注释**,后续部署 ticket 解决。本期假设 dev 路径正确。
